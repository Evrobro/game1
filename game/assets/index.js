        ...
        if (!stop) {
            ...
            let winGame = setTimeout(function () {
                if (!stop) {
                    ...
                    alert("Игра выиграна " + "\n" + nickname + ", ваш результат: " + "\n"+ "Всего было произведено: " + count + " элементов" + "\n" + "Поймано: " + catched +"\n");
                    return;
                }
            }, 10000);

            let Object = document.createElement("img");
            Object.setAttribute('src', 'assets/apple.png');
            Object.setAttribute('height', '50px');
            Object.setAttribute('width', '50px');
            Object.setAttribute('style', 'position: absolute; top: 0px;');
            Object.setAttribute('id', count.toString());
            ...
            game.insertAdjacentElement('beforeend', Object);

            visibleObjects.push([Object, setTimeout(function () {
                const moving = setInterval(function () {
                    if (!stop) {
                        ...
                        let topCount = parseInt(Object.style.top.substring(0, Object.style.top.length - 2));

                        /* Проверка на координаты */
                        if (Math.abs(roflanPomoika.getBoundingClientRect()['y'] - Object.getBoundingClientRect()["y"]) < 25) {
                            console.log("a");
                            if (Math.abs(roflanPomoika.getBoundingClientRect()['x'] - Object.getBoundingClientRect()["x"]) < 50) {
                                catched++;
                                game.removeChild(Object);
                                visibleObjects[parseInt(Object.getAttribute("id"))] = null;
                        ...
                        // Затемнение
                        if (Object.getBoundingClientRect()["y"] > roflanPomoika.getBoundingClientRect()['y']) {
                            Object.style.filter = " grayscale(100%)";
                            Object.src = 'assets/appled.png';
                        }
                        ...
                        if (ult_pressed) {
                            Object.style.left = roflanPomoika.style.left;
                        }
                        //Движение
                        topCount = topCount + getRandomArbitrary(1, 5);
                        Object.style.top = topCount.toString() + "px";
                    }
...
function execute() {
    /*  Спавн корзины */
    roflanPomoika.setAttribute('src', 'assets/buc.png');
    roflanPomoika.setAttribute('style', "position: absolute; top: 0px; left:900px;right:0px;");
    roflanPomoika.setAttribute('height', '100px');
    roflanPomoika.setAttribute('width', '100px');
    ...
    function moveRight() {
        if (!stop || !ult_pressed) {
            ...
            if (incRight + speed < game.getBoundingClientRect()['left'] + 710)
                roflanPomoika.style.left = incRight + speed + "px";
            else
                roflanPomoika.style.left = game.getBoundingClientRect()['left'] + 710 + "px";
        }
    }
...
function hideForm() {
    ...
    if (nickname !== "") {
        ...
        pauseLblStatus.innerText = "PAUSE";
